alter table public.quick_turnaround_commitments
  ...
  add column if not exists hot_reason_category text;

update public.quick_turnaround_commitments
set date_received = coalesce(date_received, created_at::date, current_date),
    hot_reason_category = coalesce(nullif(hot_reason_category, ''), 'Deadline')
where date_received is null
   or hot_reason_category is null
   or hot_reason_category = '';

alter table public.quick_turnaround_commitments
  alter column date_received set default current_date,
  alter column date_received set not null,
  alter column hot_reason_category set default 'Deadline',
  alter column hot_reason_category set not null;

alter table public.quick_turnaround_commitments
  drop constraint if exists quick_turnaround_hot_reason_category_check;

alter table public.quick_turnaround_commitments
  add constraint quick_turnaround_hot_reason_category_check
  check (hot_reason_category in ('Deadline', 'Aging', 'Customer Escalation', 'Other'));

drop view if exists public.quick_turnaround_dashboard;

create view public.quick_turnaround_dashboard
...
select
  ...
  greatest(0, extract(epoch from commitment.required_by - now()) / 3600)::numeric(12,2) as hours_remaining,
  case
...
drop function if exists public.mw_save_quick_turnaround_commitment(
  uuid, text, uuid, text, text, text, text, text, text,
  timestamptz, text, text, boolean, text, text, text, text
);

create function public.mw_save_quick_turnaround_commitment(
...
begin
  ...
  if p_required_by is null then
    raise exception 'Required completion date and time are required.';
  end if;
