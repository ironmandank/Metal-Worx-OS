...
const EMPTY_FORM = {
  ...
  requiredBy: "", requiredTime: "17:00", assignedTo: "", department: "", materialsStatus: "Not Required",
  reason: "", notes: "",
};
...
function QuickTurnaroundDashboard({ setPage, activeUser }) {
  ...
  return <Stack gap="xl">
    ...
    <Modal opened={modalOpen} onClose={() => setModalOpen(false)} title="New Quick Turnaround Commitment" centered size="lg"><Stack>
      ...
      <SimpleGrid cols={{ base: 1, sm: 2 }}><TextInput label="Customer / Requestor" value={form.customerName} onChange={(event) => updateForm("customerName", event.currentTarget.value)}/><Select label="Priority" data={["Critical", "Urgent", "High"]} value={form.priority} onChange={(value) => updateForm("priority", value || "Urgent")}/><TextInput label="Required Completion Date" type="date" required value={form.requiredBy} min={new Date().toISOString().slice(0, 10)} onChange={(event) => updateForm("requiredBy", event.currentTarget.value)}/><TextInput label="Required Time" type="time" value={form.requiredTime} onChange={(event) => updateForm("requiredTime", event.currentTarget.value)}/><Select label="Assigned To" searchable clearable data={profiles.map((profile) => profile.display_name)} value={form.assignedTo} onChange={(value) => updateForm("assignedTo", value || "")}/><Select label="Department" clearable data={["Design", "Laser", "Prep", "Welding", "Paint", "Powder", "Assembly", "QC", "Showroom", "Office"]} value={form.department} onChange={(value) => updateForm("department", value || "")}/><Select label="Material Readiness" data={["Not Required", "Needs Pricing", "Needs Ordering", "Ordered", "Partially Received", "Ready", "Blocked"]} value={form.materialsStatus} onChange={(value) => updateForm("materialsStatus", value || "Not Required")}/></SimpleGrid>
