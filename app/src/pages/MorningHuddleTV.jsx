...
const styles = `
  ...
  .tv-brand h1 { margin:0; font-size:clamp(25px,2.2vw,42px); text-transform:uppercase; }
  ...
  .tv-kpi span { color:#9ba5ac; font-size:13px; font-weight:900; text-transform:uppercase; }
  .tv-kpi strong { display:block; margin-top:8px; font-size:clamp(30px,3vw,52px); line-height:1; }
  ...
  .tv-panel h2 { display:flex; align-items:center; gap:9px; margin:0; padding:13px 16px; border-bottom:1px solid #354047; font-size:clamp(17px,1.25vw,24px); text-transform:uppercase; }
  ...
  @media(max-width:1200px){ .tv-kpis{grid-template-columns:repeat(3,1fr)} .tv-grid{grid-template-columns:1fr 1fr}.tv-panel:first-child{grid-column:1/-1} }
`;
...
function buildExecutiveSummary(data) {
  ...
  const sentences = [`Metal Worx begins today with ${projects.length} active outside project${projects.length === 1 ? "" : "s"}, ${s.activeShopJobs || 0} active shop job${s.activeShopJobs === 1 ? "" : "s"}, and ${s.todayFieldWork || 0} scheduled field activit${s.todayFieldWork === 1 ? "y" : "ies"}.`];
...
export default function MorningHuddleTV({ setPage }) {
  ...
  const priorities = (huddle.todayFocus || []).slice(0,6);
  ...
  return <div className="tv-board"><style>{styles}</style>
    ...
    <section className="tv-kpis">
      ...
      <div className="tv-kpi good"><span>Updated Today</span><strong>{updatedIds.size}</strong></div>
    ...
    <section className="tv-grid">
      <div className="tv-panel"><h2><IconClipboardCheck/> Today’s Priorities</h2>{priorities.length ? <ul className="tv-list">{priorities.map((x,i)=><li key={x.id || i}><strong>{text(x.title,"Priority")}</strong><small>{text(x.owner)} · {text(x.nextAction,"Action required")}</small></li>)}</ul>:<div className="tv-empty">No priorities recorded.</div>}</div>
      ...
      <div className="tv-panel"><h2><IconTool/> Materials & Shop</h2><ul className="tv-list"><li><strong>{data?.outsideSummary?.materialsNeedOrdered || 0} need ordering</strong><small>Projects requiring purchasing action</small></li><li><strong>{data?.outsideSummary?.materialsWaiting || 0} waiting on material</strong><small>Ordered but not fully received</small></li><li><strong>{summary.activeShopJobs || 0} active shop jobs</strong><small>Busiest: {summary.busiestDepartment || "None"}</small></li></ul></div>
    </section>
